...
void attrEventCallback(const af_lib_event_type_t eventType,
    ...
    switch (eventType) {
        ...
        case AF_LIB_EVENT_MCU_DEFAULT_NOTIFICATION:
            ...
            switch (attributeId) {
              case AF_SAMPLE_RATE:
                  // we dont have any reason to change the default so we return the default value
                  // but we MUST return something here to ensure the cloud and the MCU are in sync
                  sample_rate = *value;
                  af_lib_set_attribute_8(af_lib, AF_SAMPLE_RATE, *value);
              ...
              case AF_ALARM_TEMP:
                  ...
                  if (alarm_temp != (uint16_t)*value) {
                    af_lib_set_attribute_16(af_lib, AF_ALARM_TEMP, alarm_temp);
                    Serial.println(alarm_temp);
                  } else {
                    af_lib_set_attribute_16(af_lib, AF_ALARM_TEMP, *value);
                    Serial.println(alarm_temp);
                  }
                  break;
            }
            ...
            switch (attributeId) {
                ...
                case AF_ALARM_TEMP:
                    ...
                    if ((uint16_t)*value > 0 && (uint16_t)*value <= 400) { 
                        ...
                        af_lib_send_set_response(AF_ALARM_TEMP, true, valueLen, value);
                    } else {
                        af_lib_send_set_response(AF_ALARM_TEMP, false, valueLen, value);
                    }
                    break;
